...
public class FindPaths {
	...
	public static void main(String[] args) {
		...
		Collection<Vertex> vcopy1 = new HashSet<Vertex>();
		Collection<Vertex> vcopy2 = new HashSet<Vertex>();
		for (Vertex v0 : v) {
			vcopy1.add(v0);
		}
		for (Vertex v00 : v) {
		...
		for (Vertex v1 : vcopy1) {
			for (Vertex v2 : vcopy2) {
				Path result = g.shortestPath(v1, v2);
				System.out.println("Shortest path from " + v1.getLabel() + " to " + v2.getLabel());
				if (result.vertices.isEmpty() ) {
					System.out.println("does not exist");
				} else {	
					System.out.println(result.vertices);
